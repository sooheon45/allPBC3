<template>
    <v-card outlined>
        <v-card-title>
            OverseasDelivery # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="CountryCode" v-model="item.countryCode" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/overseasDeliveries/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
